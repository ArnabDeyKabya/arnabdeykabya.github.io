...
export function Hero() {
  ...
  const imagePath = `${process.env.NEXT_PUBLIC_BASE_PATH || ''}${profile.image}`
        ...
        <div className="max-w-6xl mx-auto">
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            ...
            <div className="space-y-8 text-center lg:text-left">
              {/* Name and Title */}
              <div className={`space-y-4 ${isVisible ? "animate-slide-in-left" : "opacity-0"}`}>
                <h1 className="text-4xl sm:text-5xl lg:text-6xl font-bold text-balance">
                  <span className="text-foreground">{profile.name}</span>
                </h1>
                <p className="text-xl sm:text-2xl text-muted-foreground font-medium">{profile.tagline}</p>
              </div>
              ...
              {/* CTA Buttons */}
              <div
                className={`flex flex-col sm:flex-row gap-4 justify-center lg:justify-start items-center ${isVisible ? "animate-slide-in-left animate-delay-300" : "opacity-0"}`}
              >
                ...
                >
                  <Link href={`${process.env.NEXT_PUBLIC_BASE_PATH || ''}/my-portfolio/cv.pdf`} target="_blank">
            ...
            <div
              className={`flex justify-center lg:justify-end ${isVisible ? "animate-slide-in-right animate-delay-200" : "opacity-0"}`}
            >
          ...
          {/* Quick Stats */}
          <div
            className={`grid grid-cols-1 sm:grid-cols-2 gap-8 pt-20 max-w-2xl mx-auto ${isVisible ? "animate-fade-in-up animate-delay-500" : "opacity-0"}`}
          >
            <div className="text-center group cursor-default hover:scale-110 transition-transform">
              <div className="text-4xl font-bold text-primary group-hover:animate-pulse">7+</div>
              <div className="text-sm text-muted-foreground">Technical Projects</div>
            </div>
            <div className="text-center group cursor-default hover:scale-110 transition-transform">
              <div className="text-4xl font-bold text-primary group-hover:animate-pulse">1</div>
